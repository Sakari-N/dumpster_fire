	...
	else
	{
		...
		else
		{
			...
			*(WCHAR*)((uintptr_t)native_name_buffer + native_name_size) = 0;
			...
			return elu_internal_win32_decode_file_path(native_path_length * sizeof(WCHAR), absolute_native_path, directory_name_length_address, buffer_size, buffer);
		}
	}
}
	...
	else
	{
		...
		else
		{
			...
			*(WCHAR*)((uintptr_t)native_name_buffer + native_name_size) = 0;
			...
			error = elu_internal_win32_decode_file_path(native_path_length * sizeof(WCHAR), absolute_native_path, &allocated_buffer_size, 0, 0);
			if (error != ENOBUFS)
				return error;
			...
			*directory_name_length_address = allocated_buffer_size;
			...
			error = elu_internal_win32_decode_file_path(native_path_length * sizeof(WCHAR), absolute_native_path, directory_name_length_address, allocated_buffer_size, buffer);
			if (error)
			{
